...
def apply_template(crm_id, template_id, data=None):
...
def action_design_import(a):
	...
	apply_template(crm_id, None, data)
...
def action_people_list(a):
	...
	if crm["owner"] == 1 and not check_crm_access(a.user.identity.id, crm_id, "view"):
		a.error(403, "Access denied")
		return
...
def action_object_get(a):
	...
	crm = get_crm(crm_id)
	if crm and crm["owner"] == 1 and not check_crm_access(a.user.identity.id, crm_id, "view"):
		a.error(403, "Access denied")
		return
...
def action_repositories_merge(a):
	...
	if crm["owner"] == 1:
		...
		if not mochi.access.check(a.user.identity.id, "crm/" + crm_id, "write"):
			a.error(403, "Write access required to merge")
			return

		server = crm["server"] or ""
		peer = mochi.remote.peer(server) if server else None
		result = mochi.remote.request(crm_id, "crm", "merge/request", {
			"crm": crm_id,
			"repo": repo_id,
			"source": source,
			"target": target,
...
def action_request_create(a):
	...
	if crm["owner"] != 1:
		return forward_to_owner(a, crm_id, "request/create", {
			"crm": crm_id, "object": a.input("object"),
	...
	broadcast_event(crm_id, "request/create", {
		"crm": crm_id, "request": request_data
	})
...
def action_request_update(a):
	...
	if crm["owner"] != 1:
		params = {"crm": crm_id, "object": a.input("object"), "request": a.input("request")}
		for k in ["repository", "source", "target", "status", "title", "description", "draft"]:
			if a.input(k) != None:
				params[k] = a.input(k)
		return forward_to_owner(a, crm_id, "request/update", params)
	...
	broadcast_event(crm_id, "request/update", {
		"crm": crm_id, "request": req
	})
...
def action_request_delete(a):
	...
	if crm["owner"] != 1:
		return forward_to_owner(a, crm_id, "request/delete", {
			"crm": crm_id, "object": a.input("object"),
