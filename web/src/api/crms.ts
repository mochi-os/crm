...
const crmsApi = {
  ...
  exportData: async (crmId: string): Promise<Blob> => {
    const response = await crmsRequest.get<Blob>(
      endpoints.crms.dataExport(crmId),
      { responseType: "blob" },
    );
  ...
  searchUsers: async (
    ...
    return crmsRequest.get(`${endpoints.crms.usersSearch}?search=${encodeURIComponent(query)}`);
  },
